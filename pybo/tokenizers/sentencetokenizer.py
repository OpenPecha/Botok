...
def join_no_verb_sentences(sentences, tokens, threshold=4):
    ...
    while i < len(sentences):
        ...
        if no_verb:
            last_word = tokens[end] if tokens[end].chunk_type == 'TEXT' else tokens[end - 1]
            if i + 1 < len(sentences) and has_last_syl(last_word, clause_boundaries):
                ...
                # join to the right
                last_word_of_previous = tokens[sentences[i-1]['end']] if tokens[sentences[i-1]['end']].chunk_type == 'TEXT' else tokens[sentences[i-1]['end'] - 1]
