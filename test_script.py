...
# assert len(custom) == 0
# main, custom = config.get_tok_data_paths('POS', modifs='./pybo/tests/trie_data/')
# assert [c for c in custom] == ['lexica_bo', 'lemmas'] == [t.parts[-1] for t in Path('./pybo/tests/trie_data/').glob('*')]
#
# # overwriting the main profile
# main, custom = config.get_tok_data_paths('./pybo/tests/trie_data/', mode='custom')
